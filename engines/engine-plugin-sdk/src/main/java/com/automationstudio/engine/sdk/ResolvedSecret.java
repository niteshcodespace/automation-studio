...
public class ResolvedSecret implements AutoCloseable {

    private static final int MAX_VALUE_LENGTH = 65_536;
    private char[] value;

    protected ResolvedSecret(char[] value) {
        this.value = value;
    }

    public static ResolvedSecret from(char[] value) {
        if (value == null || value.length == 0 || value.length > MAX_VALUE_LENGTH) {
            throw new SecretResolutionException("SECRET_VALUE_INVALID", "Resolved secret value is invalid");
        }
    ...
    public synchronized void withValue(Consumer<char[]> consumer) {
        ...
        if (value == null) {
            throw new SecretResolutionException("SECRET_VALUE_CLOSED", "Resolved secret value is closed");
        }
