...
final class GatewayHttpClient implements HttpClient {
    ...
    @Override public Response invoke(HttpRequest request){
        ...
        }catch(GatewayFailure e){recordFailure(e.code);throw e;}catch(IOException|RuntimeException e){recordFailure("GATEWAY_FAILED");throw fail("GATEWAY_FAILED");}
    }
    private void recordFailure(String code){gatewayFailure.accumulateAndGet(code,(current,next)->current==null||next.compareTo(current)<0?next:current);}
