...
/** Fixed worker bootstrap for bounded source projection and bounded Karate execution. */
public final class KarateWorkerMain {
        ...
        try(SourceProjection projection=new SourceProjection(root,32L*1024*1024,requirePosixSeal)){
            while(true){
                ...
                switch(m.type()){
                    ...
                    case "EXECUTE"->{if(!complete||executed||!m.fields().keySet().equals(Set.of("features","includeTags","excludeTags","variables","gateway","parallelism")))throw new WorkerProtocol.ProtocolException("MALFORMED_MESSAGE");int parallelism=parallelism(m.fields().get("parallelism"));executed=true;PrintStream priorOut=System.out,priorErr=System.err;KarateSequentialExecutor.Result result;try(var sink=new PrintStream(OutputStream.nullOutputStream())){System.setOut(sink);System.setErr(sink);result=new KarateSequentialExecutor().execute(root,strings(m.fields().get("features"),256,1024),strings(m.fields().get("includeTags"),32,64),strings(m.fields().get("excludeTags"),32,64),map(m.fields().get("variables"),64,1024),m.fields().get("gateway"),correlation,parallelism);}finally{System.setOut(priorOut);System.setErr(priorErr);}WorkerProtocol.write(out,new WorkerProtocol.Message("EXECUTION_RESULT",correlation,Map.of("outcome",result.outcome(),"features",String.valueOf(result.features()),"scenarios",String.valueOf(result.scenarios()),"passed",String.valueOf(result.passed()),"failed",String.valueOf(result.failed()),"diagnostic",result.diagnostic())),WorkerProtocol.DEFAULT_MAX_FRAME);}
    ...
    private static int parallelism(String value)throws WorkerProtocol.ProtocolException{try{int parsed=Integer.parseInt(value);if(parsed<1||parsed>8)throw new NumberFormatException();return parsed;}catch(NumberFormatException e){throw new WorkerProtocol.ProtocolException("MALFORMED_MESSAGE");}}
