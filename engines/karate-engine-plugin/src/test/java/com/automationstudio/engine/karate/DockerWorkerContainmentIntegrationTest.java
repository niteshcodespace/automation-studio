...
class DockerWorkerContainmentIntegrationTest {
    @Test void confinesWorkerLocalJavaAndProcessAuthority() throws Exception {
        ...
        String network="as-karate-net-"+suffix,worker="as-karate-"+suffix,gateway="as-karate-gateway-"+suffix;Process attached=null;
        try {
            ...
            if(attached!=null&&attached.isAlive())attached.destroyForcibly();bestEffort(List.of("docker","rm","-f",worker));bestEffort(List.of("docker","rm","-f",gateway));bestEffort(List.of("docker","network","rm",network));
        }
