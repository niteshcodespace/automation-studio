...
final class DockerGatewayCommand{
 ...
 static List<String> create(String name,String image,UUID id,String base,long deadline,int parallelism,WorkerLimits l,boolean testNonGlobal){if(!name.matches("^as-karate-gateway-[a-f0-9]{32}$")||!DIGEST.matcher(image).matches()||parallelism<1||parallelism>8)throw new IllegalArgumentException("Invalid gateway identity");return List.of("docker","create","--interactive","--name",name,"--label","automation-studio.execution="+id.toString().replace("-",""),"--network","bridge","--read-only","--user","10002:10002","--cap-drop","ALL","--security-opt","no-new-privileges:true","--security-opt","seccomp=builtin","--pids-limit","64","--memory","268435456","--memory-swap","268435456","--cpus","1.0","--tmpfs","/work:rw,noexec,nosuid,nodev,uid=10002,gid=10002,mode=0700,size=16777216","--env","LANG=C.UTF-8",image,id.toString(),base,String.valueOf(deadline),String.valueOf(parallelism),testNonGlobal?"test-loopback":"production");}
